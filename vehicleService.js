...
} = vehicleService;import { supabase } from './supabaseClient.js';
...
export const vehicleService = {
    ...
    async updateVehicle(vehicleId, updateData) {
        ...
        if (updateData.mileage !== undefined && updateData.mileage !== null) {
            ...
            // 3. Read the authoritative inspection relationship (verification_inspection_id)
            // Never query inspections table by vehicle_id with maybeSingle() or limit(1)
            const { data: vehicleRecord, error: fetchErr } = await supabase
                .from('vehicles')
                .select('verification_inspection_id')
                .eq('id', vehicleId)
            ...
            // 5. If verification_inspection_id is NULL, safely skip synchronization without guessing or creating inspections.
        }
    ...
    async uploadInspectionReportPdf(vehicleId, file, reportType = 'scanner') {
        ...
        // Fetch the authoritative verification_inspection_id from the vehicle record
        const { data: vehicleRecord, error: vehicleErr } = await supabase
            .from('vehicles')
            .select('verification_inspection_id')
            .eq('id', vehicleId)
