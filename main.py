...
class ReportPayload(BaseModel):
    ...
    photos: List[str] # Short-lived signed URLs generated by Edge Function
...
def generate_pdf_bytes(data: dict) -> bytes:
    ...
    meta_table_data = [
        ...
        [Paragraph("<b>Issuance UTC:</b>", body_style), Paragraph(datetime.utcnow().strftime('%Y-%m-%d %H:%M'), body_style)],
    ]
    ...
    veh_data = [
        ...
        [Paragraph("Recorded Mileage:", body_style), Paragraph(f"{vehicle.get('mileage', 0):,} km", body_style), Paragraph("Body Type:", body_style), Paragraph(str(vehicle.get('body_type', '-')), body_style)],
    ...
    counts = {"GOOD": 0, "FAIR": 0, "ATTENTION": 0, "CRITICAL": 0, "N/A": 0, "PENDING": 0}
    for item in items:
        st = (item.get('status') or 'PENDING').upper()
        if st in counts:
            counts[st] += 1
        else:
            counts["PENDING"] += 1
    ...
    sum_data = [
        ...
        [Paragraph("Total Master Checklist Items Evaluated", table_cell_style), Paragraph(str(len(items)), table_cell_style)],
        [Paragraph("Good Condition (Operational)", table_cell_style), Paragraph(str(counts["GOOD"]), table_cell_style)],
        [Paragraph("Fair Condition (Minor Wear / Note)", table_cell_style), Paragraph(str(counts["FAIR"]), table_cell_style)],
        [Paragraph("Attention Required (Scheduled Service)", table_cell_style), Paragraph(str(counts["ATTENTION"]), table_cell_style)],
        [Paragraph("Critical Safety Hazard (Immediate Action)", table_cell_style), Paragraph(str(counts["CRITICAL"]), table_cell_style)],
        [Paragraph("Not Applicable / Exempt", table_cell_style), Paragraph(str(counts["N/A"]), table_cell_style)],
    ]
    ...
    if data.get('hasScannerReport', False):
        story.append(Paragraph("<b>OBD2 Diagnostic Health Scan:</b> Verified electronic diagnostic log captured and archived securely under Dreamland diagnostic records. Reference hashes match authenticated workshop tools.", body_style))
    else:
    ...
    # ==========================================
    # 7. PHOTOGRAPHIC EVIDENCE (Using Signed URLs)
    # ==========================================
    ...
    sign_data = [
        ...
        [Paragraph("<b>Approving Administrator ID:</b>", body_style), Paragraph(str(signoff.get('approvedBy', 'N/A')), body_style)],
    ...
    # ==========================================
    # 9. VERIFICATION (Accurate Wording)
    # ==========================================
    ver_data = [
